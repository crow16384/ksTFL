...
| **stubColumns** | Spanning headers above column groups | `add_stub_column()` |
...
## Step 5: Add spanning headers (stub columns)

Create multi-level headers with `add_stub_column()`:
...
# Top-level stub spanning all columns
spec <- add_stub_column(spec, cols = c("mpg", "cyl", "hp", "wt"),
  label = "Motor Vehicle Specs",
  stubOrder = 1)

# Second-level stubs: finer grouping
spec <- add_stub_column(spec, cols = c("mpg", "cyl", "hp"),
  label = "Engine",
  stubOrder = 2)
spec <- add_stub_column(spec, cols = "wt",
  label = "Weight",
  stubOrder = 2)
```
...
- `stubOrder`: Vertical stacking order (lower = closer to top; auto-generated if NULL)
- `labelStyleRef`: Style for the stub header label

**Rules**:
- Stubs with the same `stubOrder` sit on the same row
- Overlapping columns only allowed if stubs have different `stubOrder`
...
| Add spanning header | `add_stub_column(spec, cols, label, ...)` | Controls multi-level headers |
