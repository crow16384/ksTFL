...
deterministic HarfBuzz-based pagination via `write_doc()` /
`replay_report()`
...
Examples](Reporting_Examples_with_ksTFL.Rmd) for realistic workflow
patterns. 2. [Styling Guide](Styling_Guide_with_ksTFL.Rmd) for style
primitives and reusable style systems. 3. [Column Width
Management](Column_Width_Management.Rmd) for layout tuning. 4. [Advanced
StyleRows](Advanced_StyleRows.Rmd) for conditional row actions. 5. [Font
Management](Font_Management.Rmd) for system font discovery, custom font
directories, and fallback behavior. 6. [Rendering
Pipeline](Rendering_Pipeline.Rmd) for detailed C++ architecture and DOCX
emission internals.
...
**What happens**: - Data is shadow-copied into the spec's internal
environment (not modifiable) - Columns are auto-analyzed (type
detection, width calculation) - A `TFL_spec` object is returned ready
for customization
...
is given: rendered via `ggplot2::ggsave()` to `tempdir()` using package
...
See [Reporting Examples](Reporting_Examples_with_ksTFL.Rmd) for detailed
...
`tfl_setoptions()` to make it available session-wide.

For comprehensive styling details see [Styling
Guide](Styling_Guide_with_ksTFL.Rmd).
...
Actions](Reporting_Examples_with_ksTFL.Rmd#conditional-row-actions).
...
spec <- add_span_header(spec, c(age, sex), 'Charactersitic', 
...
![](images/clipboard-2576915099.png)

*ISSUE: Currently we have a issue with span headers - the
`Treatment A (N=10)` must span all columns per the code!!!*
...
2.  **Explore [Reporting Examples](Reporting_Examples_with_ksTFL.Rmd)**
    for detailed patterns
3.  **Read [Styling Guide](Styling_Guide_with_ksTFL.Rmd)** for advanced
    styling
4.  **Read [Advanced StyleRows](Advanced_StyleRows.Rmd)** for
...
-   **Reporting Examples**: [Detailed working examples with
    explanations](Reporting_Examples_with_ksTFL.Rmd)
-   **Styling Guide**: [Comprehensive style
    reference](Styling_Guide_with_ksTFL.Rmd)
