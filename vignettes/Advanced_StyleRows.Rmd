...
- Prerequisites: complete `Getting_Started_with_ksTFL` and basic `compute_cols()` familiarity
...
**Note:** The `cols` argument to `c_merge()` must resolve to at least two **consecutive**
columns in the final report column order. The value shown in the merged cell is taken
...
**Result:** An empty separator row is inserted above each new group. To copy a
single column's value into the inserted row use `value_from` (see later examples).
Use `pos = "below"` to insert after the matching row instead.
...
`c_clear()` renders specified cells as empty (blank) in matching rows without removing the column or affecting layout. Useful for deduplication — showing a group label only on its first row and blanking it on subsequent rows.
...
`compute_cols()` accepts multiple action functions in a single call via `...`.
All actions are applied to every row matching the condition:
...
For independent conditions, use separate `compute_cols()` calls:

```{r combine-separate-conditions}
spec <- create_table(data) |>
...
- [Getting Started](Getting_Started_with_ksTFL.Rmd) — full pipeline overview and `compute_cols()` introduction
- [Styling Guide](Styling_Guide_with_ksTFL.Rmd) — styling fundamentals and built-in atoms
- [Reporting Examples](Reporting_Examples_with_ksTFL.Rmd) — complete end-to-end workflows
- [Column Width Management](Column_Width_Management.Rmd) — invisible columns for conditional logic
