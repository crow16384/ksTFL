...
2. **Helper functions**: `firstOf()`, `lastOf()`, `firstRow()`, `lastRow()`, `row_number()`, `every_nth()`, `get_names()`, `eval()`
...
- **`row_number()`**: Integer vector of row numbers (1-based)
- **`every_nth(n)`**: Logical vector TRUE at every nth row
- **`get_names(...)`**: Character vector of selected column names
- **`eval(expr)`**: Evaluate expression with tidyverse data masking

**Note**: Functions like `lag()`, `lead()`, and `n()` are NOT provided by ksTFL. If you need these, load the dplyr package.
