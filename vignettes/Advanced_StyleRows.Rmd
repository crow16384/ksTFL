...
spec <- create_table(data_groups) |>
  compute_cols(
    ...
    c_addrow(pos = "before")  # Insert an empty separator/header row
  )
```

**Result:** An empty separator row is inserted before each new group. To copy a
...
spec <- create_table(data_sales) |>
  ...
  compute_cols(
    ...
    c_addrow(pos = "after", styleRef = "summary_row")  # Insert empty subtotal row
  )
```
...
spec <- create_table(data_sales) |>
  ...
  compute_cols(
    ...
    c_addrow(pos = "before", styleRef = "section_header")
  )
```
...
spec <- create_table(data_sales) |>
  compute_cols(
    ...
    c_addrow(
      pos = "after"  # Insert an empty marker row after high-performance rows
    )
  )
```
...
spec <- create_table(data_groups) |>
  ...
  compute_cols(
    firstOf(group),
    c_addrow(pos = "before", styleRef = "group_header")
  ) |>
...
spec <- create_table(data_sales) |>
  ...
  compute_cols(
    lastOf(region),  # Last row of each region
    c_addrow(pos = "after", styleRef = "total_row")
  )
```
...
spec1 <- create_table(data[1:10, ]) |>
  add_style("elderly", s_font(bold = TRUE)) |>
  compute_cols(age > 60, c_style(age, styleRef = "elderly"))

spec2 <- create_table(data[11:20, ]) |>
  add_style("success", s_font(bold = TRUE)) |>
  compute_cols(response == "CR", c_style(response, styleRef = "success"))
...
-- **Evaluation context**: Data environment with helper functions (`lag`, `lead`, `rowNumber`, `n`)
