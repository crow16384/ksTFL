...
-   Vertical pagination budget is computed from usable page height
    minus:
    -   safety margin
...
-   Horizontal segmentation:
    ...
    -   when isColBreak is active, the renderer enforces
        ...
        repeat_header_on_each_page=true (Phase 1b constraint
        enforcement, with R warning if values are overridden)
-   Last-page footnotes:
    ...
    -   row-break mode (`allow_row_break_across_pages=true`) skips
        LastPage reshuffling and keeps natural Word row flow
    -   for `last_page`, footnote height is reserved only on the true
        final page; non-final pages keep zero footnote reserve
-   Row height model:
    -   baseline full-table row height is retained on LogicalRow
    -   segment-specific row heights are computed and stored on each
        segment
