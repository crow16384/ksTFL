...
-   Audience: users who already know the basic pipeline and want
    practical templates
...
\- [Getting Started](Getting_Started_with_ksTFL.Rmd) — pipeline overview
and all core concepts

\- [Styling Guide](Styling_Guide_with_ksTFL.Rmd) — complete style
reference and built-in atoms

\- [Advanced StyleRows](Advanced_StyleRows.Rmd) — deep dive into
`compute_cols()`, `c_glue()`, `c_clear()`

\- [Column Width Management](Column_Width_Management.Rmd) — width
locking, auto-calculation, invisible columns
...
defined columns: ![](images/clipboard-1089030957.png)
...
## ![](images/clipboard-3882147075.png)
...
## ![](images/clipboard-1432659028.png)
...
## ![](images/clipboard-959874429.png)
...
spec_mapped <- define_cols(
                spec_mapped, c(age, sex, trt),
...
![](images/clipboard-3594939568.png)
...
![](images/clipboard-2085870902.png)
...
![](images/clipboard-1444834458.png)
...
![](images/clipboard-1984427091.png)
...
## ![](images/clipboard-192293685.png)
...
![](images/clipboard-2991126070.png)
...
![](images/clipboard-3003524943.png)
...
![](images/clipboard-1437051722.png)
...
![](images/clipboard-137358296.png)
...
![](images/clipboard-2883006425.png)
...
![](images/clipboard-2551143985.png)
...
![](images/clipboard-2936122457.png)
...
`create_report()`) - `name`: Base name for the output DOCX (e.g.
`"example_report"` → `example_report.docx`) - `outDir`: Directory where
the output DOCX is written - `metaPath`: Directory where intermediate
JSON and data files are written
...
`f_combine()` (expanded) 8. End-to-end render with `write_doc()` 9.
Session defaults with `tfl_set_options()`

**Key patterns to remember**: - **Batch operations**: Use `c()` to
select multiple columns, provide single values (recycled) or N values
(one-to-one mapping) - **Multiple calls**: Chain `define_cols()`,
`add_style()`, etc. — each call merges with previous settings -
**Flexible column widths**: Auto-calculated by default; lock specific
columns with `colWidth` to trigger recalculation of others - **Combining
styles**: Use `f_combine()` for on-the-fly combinations; define named
styles for reuse - **Session defaults**: Use `tfl_set_options()` once;
all new specs inherit settings (override per-spec if needed)
