...
## 5 — Set document properties (docPrefix, hasData, content width, placement)

Goal: configure document-level properties like the document prefix (e.g., "Table 14.1"), data availability flag, and content width.

**Key function**: `set_document()`
- `docPrefix`: Prefix and number (e.g., "Table 14.1", "Listing 5.2")
- `glueNumType`: Whether to combine prefix with first title (default TRUE)
...
**Example 1: Simple document prefix**
...
**Notes**:
- Most defaults are sensible; you typically only need `set_document()` for `docPrefix` and content width
