...
<img src="figures/ksTFL-logo.svg" alt="ksTFL logo" width="700" />
...
- `write_doc(..., overrideTemplate = NULL)` keeps per-spec `docTemplate` styling.
...
**Advanced**: If you need to inspect the JSON before rendering (e.g. for debugging or CI pipelines), you can split the call into `save_report()` + `replay_report()`. See `?write_doc` and `?replay_report` for details.
