...
- **Helper functions** (`firstOf()`, `lastOf()`, `firstRow()`, `lastRow()`, `rowNumber()`, `everyNth()`, `firstOfBlock()`) are only available inside `compute_cols()` conditions — they are **not** standalone exported functions. See `vignette("Advanced_StyleRows")` for full details.
...
> **Tip**: Use `tfl_list_templates()` to discover available template names (e.g., `"Navy_Pro"`, `"Carbon_Dark"`). Use `run_styles_editor()` to interactively preview and customize templates.
