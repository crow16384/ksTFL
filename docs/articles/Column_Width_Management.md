...
### The Three Column States

ksTFL partitions table columns into three categories that determine how
...
ksTFL stores width metadata in `spec$.metadata$colWidths`:
...
This metadata enables: - Recalculation without re-parsing width
strings - Tracking which columns are locked vs. unlocked - Preserving
initial proportions for normalization
...
- **Three states**: VISIBLE/LOCKED/UNLOCKED determine width behavior
