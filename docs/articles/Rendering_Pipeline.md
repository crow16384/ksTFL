...
- Last-page footnotes:
  - `footnotePlace="last_page"` has two paths
  - deterministic mode (`allow_row_break_across_pages=false`) uses
    LastPage post-pass to keep footnotes on the true final page
  - row-break mode (`allow_row_break_across_pages=true`) skips LastPage
    reshuffling and keeps natural Word row flow
  - for `last_page`, footnote height is reserved only on the true final
    page; non-final pages keep zero footnote reserve
- Row height model:
