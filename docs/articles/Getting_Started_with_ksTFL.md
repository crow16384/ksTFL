...
exist and be readable) - If a **ggplot2 object** is given: rendered via
[`ggplot2::ggsave()`](https://ggplot2.tidyverse.org/reference/ggsave.html)
to [`tempdir()`](https://rdrr.io/r/base/tempfile.html); the resulting
path is stored in the spec - Minimal metadata is created; the file is
copied to the output directory when saved
...
auto-recalculate to sum to 100% - Disable with
...
[`add_footer()`](https://example.com/reference/add_footer.md) take 3
arguments: left-aligned, center-aligned, right-aligned - Placeholders
like `{page}` are filled in by the renderer - Each call **replaces**
previous headers/footers (does not append) - Multiple
...
**Parameters**: - `report`: A `TFL_report` object - `docFileName`:
Output file name (e.g., “report.docx”) - `outDir`: Directory for final
DOCX (where renderer writes output) - `metaPath`: Directory for JSON
metadata + data files (consumed by renderer) - `prettify`: If TRUE,
formats JSON nicely (useful for debugging)

**Output files**: - `{metaPath}/{spec_hash}.json` — Main specification
(consumed by renderer) - `{metaPath}/{dataRef}.json` — Table data files
(one per table spec) - Figure files copied to `metaPath` as-is

**Return value**: List with: - `spec_file`: Path to generated spec
JSON - `data_files`: Named list of data file paths - `metadata`: Report
...
| **Styles from f_combine() not consolidated** | Call [`create_report()`](https://example.com/reference/create_report.md) before [`save_report()`](https://example.com/reference/save_report.md) |
