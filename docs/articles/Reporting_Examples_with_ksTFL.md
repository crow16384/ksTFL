...
- Export artifacts for rendering with
  [`save_report()`](https://example.com/reference/save_report.md)
  (writes JSON and data/figure files into `metaPath`).
...
Goal: build a clinical-style table with multi-level column headers,
...
(e.g., `stubOrder = 1` above `stubOrder = 2`) - You can have multiple
stubs at the same order level as long as their column sets don’t
overlap - If NULL, `stubOrder` is auto-generated starting from 1
...
Goal: show
[`save_report()`](https://example.com/reference/save_report.md) usage —
it validates and serializes specs and writes metadata/data/figure files
to `metaPath`.

### What `save_report()` does

[`save_report()`](https://example.com/reference/save_report.md) is the
final step in the reporting pipeline. It: 1. **Assembles** multiple
specs into a single report (if not already done via
[`create_report()`](https://example.com/reference/create_report.md)) 2.
**Validates** the complete report against JSON Schema 3. **Serializes**
to JSON, replacing internal references with file paths 4. **Writes
files** to `metaPath`: - `{spec_hash}.json` — the main metadata spec
file (consumed by the renderer) - `{dataRef}.json` — table data files
(one per table spec) - Figure files — copied from input paths to
`metaPath` 5. **Returns metadata** with generation timestamps and file
locations
...
**Parameters**: - `report`: A `TFL_report` object (created via
[`create_report()`](https://example.com/reference/create_report.md)) -
`docFileName`: Output DOCX filename (passed to renderer) - `outDir`:
Directory where output DOCX will be written - `metaPath`: Directory
where metadata and data files are written (renderer consumes these) -
`prettify`: If TRUE, formats JSON output nicely (useful for debugging)

**Return value**: A list with: - `spec_file`: Full path to generated
spec JSON - `data_files`: Named list of generated data file paths -
`metadata`: Report-level metadata (timestamps, paths)

Notes: [`save_report()`](https://example.com/reference/save_report.md)
is the public entrypoint to persist specs/data/figures for downstream
rendering. In a real workflow,
[`render_docx()`](https://example.com/reference/render_docx.md) consumes
the spec JSON to produce the final .docx file.
...
8. End-to-end export with
[`save_report()`](https://example.com/reference/save_report.md) 9.
...
[`?save_report`](https://example.com/reference/save_report.md), etc.
