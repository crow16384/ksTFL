...
- Processed before
  ...
  in the rendering chain, so the cleared state participates in
  subsequent actions. In particular: combining `c_clear()` +
  [`c_glue()`](https://crow16384.github.io/ksTFL-release/reference/c_glue.md)
  on the same column effectively *replaces* the original cell content
  with the glued value.

- Compatible with
  [`c_style()`](https://crow16384.github.io/ksTFL-release/reference/c_style.md):
  styling is independent of text content.

- When
  [`c_merge()`](https://crow16384.github.io/ksTFL-release/reference/c_merge.md)
  targets a cleared leader cell, the merged span renders as a blank
  merged cell.
