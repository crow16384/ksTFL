...
static voidpf ft_gzip_alloc(voidpf opaque, uInt items, uInt size) {
...
static void ft_gzip_free(voidpf opaque, voidpf address) {
  FT_Memory memory = (FT_Memory)opaque;
...
static FT_Error ft_gzip_check_header(FT_Stream stream) {
  ...
  if (FT_STREAM_SEEK(0) || FT_STREAM_READ(head, 4)) goto Exit;
  ...
  if (head[3] & FT_GZIP_EXTRA_FIELD) {
    ...
    if (FT_READ_USHORT_LE(len) || FT_STREAM_SKIP(len)) goto Exit;
  }
...
static FT_Error ft_gzip_file_init(FT_GZipFile zip, FT_Stream stream, FT_Stream source) {
  ...
  if (inflateInit2(zstream, -MAX_WBITS) != Z_OK || !zstream->next_in) error = FT_THROW(Invalid_File_Format);
...
static FT_Error ft_gzip_file_fill_input(FT_GZipFile zip) {
  ...
  if (stream->read) {
    ...
    if (size > FT_GZIP_BUFFER_SIZE) size = FT_GZIP_BUFFER_SIZE;
...
static FT_Error ft_gzip_file_fill_output(FT_GZipFile zip) {
  ...
  while (zstream->avail_out > 0) {
    ...
    if (err == Z_STREAM_END) {
      ...
      if (zip->limit == zip->cursor) error = FT_THROW(Invalid_Stream_Operation);
      break;
    } else if (err != Z_OK) {
      zip->limit = zip->cursor;
...
static FT_Error ft_gzip_file_skip_output(FT_GZipFile zip, FT_ULong count) {
  FT_Error error = FT_Err_Ok;
...
static FT_ULong ft_gzip_file_io(FT_GZipFile zip, FT_ULong pos, FT_Byte *buffer, FT_ULong count) {
  FT_ULong result = 0;
  FT_Error error;
...
static void ft_gzip_stream_close(FT_Stream stream) {
  ...
  if (!stream->read) FT_FREE(stream->base);
}

static unsigned long ft_gzip_stream_io(FT_Stream stream, unsigned long offset, unsigned char *buffer,
                                       unsigned long count) {
  FT_GZipFile zip = (FT_GZipFile)stream->descriptor.pointer;
...
FT_Gzip_Uncompress(FT_Memory memory, FT_Byte *output, FT_ULong *output_len, const FT_Byte *input, FT_ULong input_len) {
  ...
  if (!memory || !output_len || !output) return FT_THROW(Invalid_Argument);
  ...
  if (err != Z_OK) return FT_THROW(Invalid_Argument);
  ...
  if (err == Z_MEM_ERROR) return FT_THROW(Out_Of_Memory);

  if (err == Z_BUF_ERROR) return FT_THROW(Array_Too_Large);

  if (err == Z_DATA_ERROR) return FT_THROW(Invalid_Table);

  if (err == Z_NEED_DICT) return FT_THROW(Invalid_Table);
