...
struct TestResult {
    ...
    Rcpp::List to_list() const {
        return Rcpp::List::create(
            Rcpp::Named("passed") = Rcpp::wrap(passed),
...
Rcpp::List cpp_test_units() {
    ...
    t.check_throw([](){ Color::parse("XYZ"); },     "Color::parse invalid chars throws");
    t.check_throw([](){ Color::parse("#12345"); },   "Color::parse 5-char hex throws");
    t.check_throw([](){ Color::parse("#1234567"); }, "Color::parse 7-char hex throws");
    t.check_throw([](){ Color::parse("#GGGGGG"); },  "Color::parse non-hex G throws");
    ...
    // --- border_line_style_to_ooxml ---
    {
        ...
        t.check_eq(std::string(border_line_style_to_ooxml(BorderLineStyle::Single)),     std::string("single"), "OOXML border Single");
        t.check_eq(std::string(border_line_style_to_ooxml(BorderLineStyle::Double)),     std::string("double"), "OOXML border Double");
        t.check_eq(std::string(border_line_style_to_ooxml(BorderLineStyle::Dashed)),     std::string("dashed"), "OOXML border Dashed");
        t.check_eq(std::string(border_line_style_to_ooxml(BorderLineStyle::Dotted)),     std::string("dotted"), "OOXML border Dotted");
        t.check_eq(std::string(border_line_style_to_ooxml(BorderLineStyle::Thick)),      std::string("thick"),  "OOXML border Thick");
        t.check_eq(std::string(border_line_style_to_ooxml(BorderLineStyle::Wave)),       std::string("wave"),   "OOXML border Wave");
        t.check_eq(std::string(border_line_style_to_ooxml(BorderLineStyle::DashSmallGap)), std::string("dashSmallGap"), "OOXML border DashSmallGap");
    }
...
Rcpp::List cpp_test_inline_parser() {
    ...
    // --- Nested: <b><i>both</i></b> → bold AND italic ---
    {
        ...
        for (const auto& run : cell.paragraphs[0].runs) {
            if (run.text == "both"
                && run.style.bold_override
                && run.style.italic_override) found = true;
        }
...
Rcpp::List cpp_test_xml_writer() {
    ...
    // --- Nested elements ---
    {
        ...
        w.end_element();      // w:p
        t.check_eq(w.str(),
                   std::string("<w:p><w:r><w:t>text</w:t></w:r></w:p>"),
