...
void DocxEmitter::emit_table_header(
    XmlWriter &w, const HeaderGrid &header_grid,
    const HorizontalSegment &segment, const StyleResolver &resolver,
  ...
  for (size_t row_idx = 0; row_idx < total_header_rows; ++row_idx) {
    ...
    if (row_idx < header_grid.row_heights.size() &&
      ...
      w.attribute("w:val",
                  std::to_string(header_grid.row_heights[row_idx].to_twips()));
    ...
    for (const auto &cell : header_row) {
      ...
      if (visible_span > 0) {
        ...
        if (is_first_header_row &&
            tmpl.table_style.structural.header_top_border.has_value()) {
          if (!tcp.borders.has_value()) {
            tcp.borders = Borders{};
          }
          tcp.borders->top = tmpl.table_style.structural.header_top_border;
        }
        if (is_last_header_row &&
            tmpl.table_style.structural.header_bottom_border.has_value()) {
          if (!tcp.borders.has_value()) {
            tcp.borders = Borders{};
          }
          tcp.borders->bottom =
...
void DocxEmitter::emit_table_row(
    XmlWriter &w, const LogicalRow &row, Length row_height,
    const HorizontalSegment &segment, const TFLSpec &spec,
    const StyleResolver &resolver, bool is_last_row,
    const std::unordered_map<size_t, int64_t> &col_widths,
    const std::unordered_set<size_t> &seg_cols) const {
  const auto &tmpl = resolver.template_styles();
  w.start_element("w:tr");
  ...
  for (size_t col_idx : segment.column_indices) {
    ...
    if (col_idx < spec.columns.size()) {
      ...
      cell_style = resolver.resolve_body_cell_style(
          spec.columns[col_idx], row.row_style_ref, std::nullopt, std::nullopt,
          is_addrow);
      if (cell.style_ref.has_value()) {
    ...
    if (is_last_row &&
        tmpl.table_style.structural.table_bottom_border.has_value()) {
      if (!tcp.borders.has_value()) {
        tcp.borders = Borders{};
      }
...
void DocxEmitter::emit_table(XmlWriter &w, const TFLSpec &spec,
  ...
  if (tmpl.table_style.table_borders.has_value()) {
    ...
    auto emit_border = [&](const char *name, const std::optional<Border> &b) {
      ...
      w.attribute("w:val",
                  b->line_style.has_value()
                      ? border_line_style_to_ooxml(b->line_style.value())
                      : "single");
  ...
  std::optional<Length> top_empty_line =
      spec.document.top_empty_line.has_value()
          ? spec.document.top_empty_line
          : tmpl.table_style.top_empty_line;
  std::optional<Length> bottom_empty_line =
      spec.document.bottom_empty_line.has_value()
          ? spec.document.bottom_empty_line
          : tmpl.table_style.bottom_empty_line;

  auto emit_empty_spacer_row = [&](const Length &spacer_height,
                                   bool apply_table_bottom_border) {
    if (spacer_height.emu <= 0)
      return;
    ...
    size_t first_col =
        segment.column_indices.empty() ? 0 : segment.column_indices.front();
    StyleDef cell_style;
    if (first_col < spec.columns.size()) {
      cell_style = resolver.resolve_body_cell_style(spec.columns[first_col],
  ...
  for (size_t ri = page.first_row; ri <= page.last_row && ri < rows.size();
       ++ri) {
    if (rows[ri].type == LogicalRowType::GroupBreak)
      continue;
  ...
  bool use_bottom_spacer = has_body_rows && bottom_empty_line.has_value() &&
                           bottom_empty_line->emu > 0;
  ...
  for (size_t ri = page.first_row; ri <= page.last_row && ri < rows.size();
    ...
    Length rh = (ri < segment.row_heights.size()) ? segment.row_heights[ri]
                                                  : rows[ri].measured_height;
    emit_table_row(w, rows[ri], rh, segment, spec, resolver, is_last,
                   col_widths, seg_cols);
  }
