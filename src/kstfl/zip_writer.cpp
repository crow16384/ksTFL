...
ZipWriter::ZipWriter(const std::string &path) {
  ...
  if (!zip_handle_) { throw RenderError("Failed to open ZIP file for writing: " + path); }
}
...
static void write_entry(zipFile zf, const std::string &entry_path, const void *data, size_t size) {
  zip_fileinfo fi;
  std::memset(&fi, 0, sizeof(fi));

  int err = zipOpenNewFileInZip(zf, entry_path.c_str(), &fi, nullptr, 0, // extra field (local)
                                nullptr, 0,                              // extra field (global)
                                nullptr,                                 // comment
                                Z_DEFLATED, Z_DEFAULT_COMPRESSION);
  if (err != ZIP_OK) { throw RenderError("Failed to open ZIP entry: " + entry_path); }
  ...
  if (err != ZIP_OK) { throw RenderError("Failed to close ZIP entry: " + entry_path); }
}
...
void ZipWriter::add_entry(const std::string &entry_path, const std::string &content) {
  if (closed_) { throw RenderError("Cannot add entry to closed ZIP file"); }
...
void ZipWriter::add_binary_entry(const std::string &entry_path, const std::vector<uint8_t> &content) {
  if (closed_) { throw RenderError("Cannot add entry to closed ZIP file"); }
  write_entry(zip_handle_, entry_path, content.data(), content.size());
}
...
void ZipWriter::add_file(const std::string &entry_path, const std::string &file_path) {
  ...
  if (!ifs) { throw RenderError("Cannot read file for ZIP inclusion: " + file_path); }
...
void ZipWriter::close() {
  ...
  if (zip_handle_) {
    ...
    zip_handle_ = nullptr;
    if (err != ZIP_OK) { throw RenderError("Failed to finalize ZIP file (error " + std::to_string(err) + ")"); }
  }
}
