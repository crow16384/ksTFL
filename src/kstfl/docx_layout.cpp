...
void DocxEmitter::emit_header_footer_section(XmlWriter &w, const std::vector<HeaderFooterRow> &rows,
                                             const StyleDef &style, Length usable_width, size_t page_num,
                                             size_t total_pages, bool use_fields) const {
    ...
    if (style.paragraph.has_value()) {
      ...
      if (style.paragraph->spacing.has_value()) {
        ...
        if (style.paragraph->spacing->before.has_value()) {
          w.attribute("w:before", std::to_string(style.paragraph->spacing->before->to_twips()));
        }
        if (style.paragraph->spacing->after.has_value()) {
          w.attribute("w:after", std::to_string(style.paragraph->spacing->after->to_twips()));
        }
        if (style.paragraph->spacing->line_spacing_multiplier.has_value()) {
          int line_val = static_cast<int>(style.paragraph->spacing->line_spacing_multiplier.value() * 240.0);
...
std::string DocxEmitter::emit_hdr_ftr_xml_part(const std::vector<HeaderFooterRow> &rows, const StyleDef &style,
                                               Length usable_w, const char *root_element,
...
void DocxEmitter::emit_section_props(XmlWriter &w, const PageConfig &page, const std::string &header_rid,
                                     const std::string &footer_rid, bool continuous, bool is_body_level) const {
  ...
  if (page.orientation == Orientation::Landscape) { w.attribute("w:orient", "landscape"); }
  w.end_element();
