...
void DocxEmitter::build_hdr_ftr_parts(const TFLDocument &doc, std::vector<HdrFtrPartInfo> &all_hdr_ftr_parts,
  ...
  for (size_t spec_idx = 0; spec_idx < doc.specs.size(); ++spec_idx) {
    ...
    if (!spec.headers.empty()) {
      ...
      std::string xml = emit_hdr_ftr_xml_part(spec.headers, header_style, usable_width, "w:hdr");
