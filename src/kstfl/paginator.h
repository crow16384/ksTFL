...
public:
  ...
  static PaginationResult paginate(const TFLSpec &spec, std::vector<LogicalRow> &rows, const HeaderGrid &header_grid,
...
private:
  ...
  static std::vector<HorizontalSegment> build_segments(const std::vector<ColumnSpec> &columns);
  ...
  static std::vector<Length> compute_row_heights(const std::vector<LogicalRow> &rows,
  ...
  static std::unordered_map<size_t, int64_t> compute_segment_column_widths(const std::vector<ColumnSpec> &columns,
  ...
  static std::vector<Length> compute_segment_row_heights(const std::vector<LogicalRow> &rows,
