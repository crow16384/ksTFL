...
void DocxEmitter::stamp_exact_line_height(StyleDef &style) const {
  if (!measurer_)
    return; // no measurer available, skip
  if (!style.font)
    return; // no font info, skip
  ...
  if (!style.paragraph->spacing)
    style.paragraph->spacing = SpacingProps{};
...
void DocxEmitter::emit_run_props(XmlWriter &w, const FontProps &font,
  ...
  bool underline =
      font.underline.value_or(false) || run_style.underline_override;
  if (underline) {
...
void DocxEmitter::emit_para_props(XmlWriter &w,
                                  const ParagraphProps &pp) const {
  ...
  if (pp.alignment.has_value()) {
    w.element_with_attr("w:jc", "w:val",
                        alignment_to_ooxml(pp.alignment.value()));
  }
  ...
  if (pp.spacing.has_value()) {
    ...
    if (pp.spacing->exact_line_height.has_value()) {
      ...
      int line_val =
          static_cast<int>(pp.spacing->line_spacing_multiplier.value() * 240.0);
  ...
  if (pp.indents.has_value()) {
    ...
    if (pp.indents->first_line.has_value()) {
      w.attribute("w:firstLine",
                  std::to_string(pp.indents->first_line->to_twips()));
    }
...
void DocxEmitter::emit_cell_props(XmlWriter &w, const TableCellProps &tcp,
                                  Length cell_width, int grid_span,
  ...
  if (tcp.vertical_alignment.has_value()) {
    ...
    w.element_with_attr("w:vAlign", "w:val",
                        (it != valign_map.end()) ? it->second : "top");
  }
  ...
  if (tcp.text_orientation.has_value()) {
    static const std::unordered_map<TextOrientation, const char *> orient_map{
        {TextOrientation::BottomToTop, "btLr"},
        {TextOrientation::TopToBottom, "tbRl"}};
    auto oit = orient_map.find(tcp.text_orientation.value());
    if (oit != orient_map.end()) {
      w.element_with_attr("w:textDirection", "w:val", oit->second);
    }
...
void DocxEmitter::emit_paragraph(XmlWriter &w, const std::string &text,
...
void DocxEmitter::emit_text_groups(
    XmlWriter &w, const std::vector<TextGroup> &groups,
  ...
  for (const auto &group : groups) {
    ...
    if (group.toc_level > 0) {
      toc_style_id =
          find_toc_heading_style_id(toc_headings, style, group.toc_level);
    }
...
void DocxEmitter::emit_text_groups_combined(
    XmlWriter &w, const std::vector<TextGroup> &groups,
    const StyleDef &base_style, const StyleResolver &resolver,
