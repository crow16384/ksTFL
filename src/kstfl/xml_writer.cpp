...
void XmlWriter::end_element() {
  if (tag_stack_.empty()) { throw RenderError("XmlWriter::end_element() called with empty tag stack"); }
...
void XmlWriter::attribute(const std::string &name, const std::string &value) {
  if (!start_tag_open_) { throw RenderError("XmlWriter::attribute() called outside of start tag"); }
  buffer_ += ' ';
  buffer_ += name;
  buffer_ += "=\"";
...
void XmlWriter::comment(const std::string &text) {
  ...
  for (size_t pos = sanitized.find("--"); pos != std::string::npos; pos = sanitized.find("--", pos + 2)) {
...
void XmlWriter::element_with_text(const std::string &name, const std::string &content) {
  ...
  if (name == "w:t") { attribute("xml:space", "preserve"); }
  text(content);
  end_element();
}

void XmlWriter::element_with_attr(const std::string &name, const std::string &attr_name,
...
void XmlWriter::element_with_attr(const std::string &name, const std::string &attr_name, int64_t attr_value) {
